...
public class Ac1parte2Application {
	 ...
	 public CommandLineRunner init(
		 ...
		 return args -> {
			 ...
			 System.out.println("*** PRODUTOS QUE COMEÇAM COM 'SMARTPHONE' ***");
			List<Produto> produtosNomeS = produtoRepository.findByNomeStartingWith("Smartphone");
			 produtosNomeS.forEach(p -> System.out.println(p.getNome()));
			 
			 
			System.out.println("*** CATEGORIAS QUE COMEÇAM COM 'ELETRONICOS' ***");
			List<Categoria> categoriasComE = categoriaRepository.findByNomeStartingWith("Eletrônicos");
			categoriasComE.forEach(c -> System.out.println(c.getNome()));

			
			System.out.println("*** PRODUTOS DA CATEGORIA COM ID = 2 ***");
			Categoria categoriaComProdutos = categoriaRepository.findByIdWithProdutos(2L);
